...
from experimental.expansion import load_implicit_graph

# remove prefix only (do NOT drop clauses; do NOT keep only "who ..." part)
...
class TKGQAPipeline:
    ...
    def process(
        ...
        # Optional override (oracle/fallback eval only): replace extracted entities entirely
        ...
        # Keep extracted_entities key consistent with what downstream uses (post-override view)
        results["extracted_entities"] = [e["name"] for e in entities]


        # For single-event we use gold_time_anchor.
        if single_event:
        ...
        # Step 2: Expand entities (currently skipped)
        ...
        if use_time_filter:
            if single_event and anchor_ts:
                ...
                mode = single_event_time_mode  # usually "around"
                ...
                # Your original extractor-date path (kept for completeness)
                mode = _signal_to_time_mode(signal_type)
                results["time_filter_mode"] = mode
